...
use App\Modules\Scheduling\Application\CancelAppointmentData;
...
/**
 * Validate and normalize a patient appointment cancellation command.
 */
final class CancelAppointmentRequest extends FormRequest
{
    private const ALLOWED_INPUTS = ['reason_code', 'idempotency_key', 'if_match'];
    ...
    public function rules(): array
    {
        return [
            'reason_code' => [
                'required',
                'string',
                Rule::enum(AppointmentCancellationReason::class),
            ],
            'idempotency_key' => [
                'required',
                'string',
     ...
     */
    public function toData(): CancelAppointmentData
    {
        ...
        if (! is_string($requestPublicId) || $version === null) {
            throw new LogicException('A validated appointment cancellation requires request and version identifiers.');
        }

        return new CancelAppointmentData(
            reason: AppointmentCancellationReason::from((string) $this->validated('reason_code')),
